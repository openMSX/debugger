...
ExpanderCorner::ExpanderCorner(WidgetDecorator* parent, Qt::Corner location)
    ...
    , dragaction{undecidedDrag}, dragorientation{Qt::Horizontal}
    , internalOverlay{nullptr}, externalOverlay{nullptr}
 ...
 * If the orientation is orthogonal we replace the widget with a new splitter and have the widget added to the new splitter
 */
...
{
    auto* switchwdg = dynamic_cast<SwitchingWidget*>(parentDecorator->layout()->itemAt(0)->widget());
...
void ExpanderCorner::setupJoiners(WidgetDecorator* parentDecorator, BlendSplitter* parentSplitter, int x, int y, Qt::Orientation /*splitorientation*/)
{
    ...
    if (isInContinuationOfSplitter(parentSplitter, pos().x() + x, pos().y() + y)) {
        ...
        } else if (!isOnTrailingHandler(parentSplitter) &&
...
void ExpanderCorner::followDragJoiners(WidgetDecorator* parentDecorator, BlendSplitter* parentSplitter, int x, int y, Qt::Orientation /*splitorientation*/)
{
    ...
    if (isInContinuationOfSplitter(parentSplitter, x, y)) {
        ...
        if ((isOnTrailingHandler(parentSplitter) && pickCoordinate(x, y, o) > pickSize(parentDecorator->size(), o))
                || (!isOnTrailingHandler(parentSplitter) && pickCoordinate(x, y, o) < 0)) {
            externalOverlay->show();
            internalOverlay->hide();
            setCursor(parentSplitter->orientation() == Qt::Horizontal ? Qt::SizeHorCursor : Qt::SizeVerCursor);
        } else if ((isOnTrailingHandler(parentSplitter) && pickCoordinate(x, y, o) <= pickSize(parentDecorator->size(), o))
                   || (!isOnTrailingHandler(parentSplitter) && pickCoordinate(x, y, o) >= 0)) {
...
bool ExpanderCorner::isInContinuationOfSplitter(BlendSplitter* parentSplitter, int x, int y)
{
    if (parentSplitter->orientation() == Qt::Horizontal
        && y > 0 && y < parentSplitter->height()) {
        return true;
    }
    if (parentSplitter->orientation() == Qt::Vertical
        && x > 0 && x < parentSplitter->width()) {
        return true;
    }
    return false;
}
...
void ExpanderCorner::decideDragAction(QMouseEvent* event, WidgetDecorator* parentDecorator, BlendSplitter* parentSplitter)
{
    ...
    if (abs(x) < BlendSplitter::expanderSize && abs(y) < BlendSplitter::expanderSize) {
        return;
    }
    ...
    if (x * unitX > 0 && y * unitY > 0 &&
            ((dragorientation == Qt::Horizontal && parentDecorator->width() > 2 * BlendSplitter::expanderSize) ||
             (dragorientation == Qt::Vertical && parentDecorator->height() > 2 * BlendSplitter::expanderSize))) {
...
void ExpanderCorner::mouseMoveEvent(QMouseEvent* event)
{
    ...
    auto* parentSplitter = qobject_cast<BlendSplitter*>(parentDecorator->parentWidget());
    if (!parentSplitter) {
...
void ExpanderCorner::mouseReleaseEvent(QMouseEvent* event)
{
    ...
    if (event->button() != Qt::LeftButton || dragaction != joinDrag) {
        return;
    }
    ...
    auto* toKeepDecorator = qobject_cast<WidgetDecorator*>(widgetToKeep);
    if (!toKeepDecorator) {
        qCritical("A BlendSplitter library error occurred. Error code: 1");
        return;
    }
    auto* parentSplitter = qobject_cast<BlendSplitter*>(externalOverlay->parentWidget()->parentWidget());
    if (!parentSplitter) {
